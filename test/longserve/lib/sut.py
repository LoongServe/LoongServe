...
def get_input_ids(model_dir: str, num_tokens: int) -> torch.Tensor:
    ...
    if stem_token_ids is None:
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=False)   # Don't use fast tokenizer, otherwise huggingface will warn us when calling LlamaTpPartModel later
