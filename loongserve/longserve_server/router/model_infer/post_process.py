...
from loongserve.longserve_server.router.model_infer.infer_batch import InferBatch, InferReq
from loongserve.common.basemodel.triton_kernel.apply_penalty import apply_penalty

def sample(logits: torch.Tensor, reqs: list[InferReq]):
    logits = logits.contiguous()    # [batch_size, vocab_size]
    if not any([req.sampling_param.do_sample for req in reqs]):
        # All requests are using greedy sampling
        # We use an optimizied code
        return logits.argmax(dim=-1), logits.max(dim=-1).values
